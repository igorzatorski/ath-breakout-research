...
def test_loads_only_requested_parquet_columns(tmp_path) -> None:
    file_path = tmp_path / "AAPL.parquet"
    data = pd.DataFrame({"date": ["2024-01-02"], "close": [100.0]})
    save_security_data(data, file_path)

    result = load_security_data(file_path, columns=["close"])
